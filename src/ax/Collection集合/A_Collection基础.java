...
// 【collection集合是set和list的父接口，就是说无论是set还是list都可以用collection】
/*
 * 集合的由来：
 ...
 * 2:删除功能
 * 		void clear():移除所有元素
 * 		boolean remove(Object o):移除一个元素	// 【返回值是是否移除成功】
 * 		boolean removeAll(Collection c):移除一个集合的元素(是一个还是所有)		// 【返回值是是否移除成功】
 * 3:判断功能
 * 		boolean contains(Object o)：判断集合中是否包含指定的元素
 * 		boolean containsAll(Collection c)：判断集合中是否包含指定的集合元素(是一个还是所有)
 * 		boolean isEmpty()：判断集合是否为空
 * 4:获取功能
 * 		Iterator<E> iterator()(重点)	// 【后面会重点讲解】
 * 5:长度功能
 * 		int size():元素的个数【☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆】
 * 		集合没有length()方法 【☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆】
 * 6:交集功能
 ...
 * 		Object[] toArray()
 */
public class A_Collection基础 {
	public static void main(String[] args) {
		// 测试不带All的方法

		// 创建集合对象
		// Collection c = new Collection(); //错误，因为接口不能实例化
		Collection<String> c = new ArrayList<String>();		// 【多态，尖括号是泛型，后面会详细讲解】
